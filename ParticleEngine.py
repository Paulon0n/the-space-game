...
class Emitter(object):
    emitters = []
    def __init__(self, pos, ang, debType = 0,  max=25):
        if debType==3:
            self.pos = Vex(0, 0, 0)
        else:
            self.pos = pos.copy()
        ...
        if debType == 0:
            max = 25
            self.particles = [Particle(self,pi+ang+random.uniform(-.5, .5), random.uniform(.1, .2), debType) for x in range(max)]
        elif debType == 1:
            max = 50
            self.dur = 5000
            self.particles = [Particle(self,pi+ang+random.uniform(-pi, pi), random.uniform(.1, .5), debType, (200, 0, 0)) for x in range(max)]
        elif debType == 2:
            max = 25
            self.dur = 5000
            self.particles = [Particle(self,pi+ang+random.uniform(-.5, .5), random.uniform(.01, .2), debType, (0, 200, 0)) for x in range(max)]
        elif debType == 3:
            max = 10
            self.dur = 200
            self.particles = [Particle(self,pi+ang+random.uniform(-.5, .5), random.uniform(.01, .2), debType, (255, 0, 0)) for x in range(max)]
