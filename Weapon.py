...
class Weapon:
    ...
    def __init__(self, parent, rotation=[0, 0, 0], scale=[1, 1, 1], speed=1.2, damage=1, shape=1, color=[255, 0, 0], offset = 2):
        self.parent = parent
    ...
    #def __  ( pos=[0, 0, 0],            rot=     ,       scale,  angle_rad=0, speed=1, damage=0, shape=0, color=[255, 0, 0]):
        if self.updateTimer() and self.updateTemp():
            ...
            self.getSpeed(), self.totalDamage(), self.getShape(), self.getColor(), self.getOffset())
